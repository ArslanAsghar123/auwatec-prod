...
/**
 * Class to backup full system
 *
 * @package Swpa\SwpaBackup\Procedures
 * @license See COPYING.txt for license details
 * @author  swpa <info@swpa.dev>
 */
class SystemWithoutMediaBackup extends Procedure
{
    /**
     * backup filename prefix
     * @var string
     */
    protected string $filePrefix = 'backup.system-without-media.';
    
    /**
     * run backup of system without media
     *
     * @throws DatabaseTypeNotSupported
     * @throws FilesystemTypeNotSupported|ConfigException
    ...
    public function run(): void
    {
        ...
        $this->logger->info('start procedure: backup of system (without media)');
        ...
        $excluded = [
            ...
            $this->config->getProjectDirectory('public/media')
        ];
        ...
        try {
            $sequence->execute();
            $this->addBackupToLog('Backup of system without media', true, $destinations);
        } catch (Exception $e) {
            $this->addBackupToLog('Backup of system without media. ERROR!: ' . $e->getMessage(), false, $destinations);
            unlink($workingFile);
        }
        
        $this->logger->info('end of procedure: backup of system (without media)');
    }
