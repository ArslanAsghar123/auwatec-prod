...
use Swpa\SwpaBackup\Service\Archive\Helper\File;

class Tar extends AbstractArchive implements ArchiveInterface
{
    /**
     * Tar block size
     *
    ...
    protected function _createTar($skipRoot = false, $finalize = false)
    {
        ...
        if (is_dir($file)) {
            ...
            if (false === $dirFiles) {
                throw new \Exception('Can\'t scan dir: ' . $file);
            }
            ...
            foreach ($dirFiles as $item) {
                if (!$this->isExcluded($file . $item) && stripos($file . $item, '.git') === false) {
        ...
        while (!$archiveReader->eof()) {
            ...
            if (in_array($header['type'], ["0", chr(0), ''])) {
                ...
                if (!file_exists($dirname)) {
                    ...
                    if (false === $mkdirResult) {
                        throw new \Exception('Failed to create directory ' . $currentFile);
                    }
                }
                $list[] = $currentFile . '/';
