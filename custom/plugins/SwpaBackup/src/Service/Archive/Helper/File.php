...
namespace Swpa\SwpaBackup\Service\Archive\Helper;
    ...
    public function open($mode = 'w+', $chmod = null)
    {
        ...
        if ($this->_isInWriteMode) {
            if (!is_writable($this->_fileLocation)) {
                throw new \Exception('Permission denied to write to ' . $this->_fileLocation);
            }
            
            if (is_file($this->_filePath) && !is_writable($this->_filePath)) {
                throw new \Exception("Can't open file {$this->_fileName} for writing. Permission denied.");
            }
        }
        
        if ($this->_isReadableMode($mode) && (!is_file($this->_filePath) || !is_readable($this->_filePath))) {
            if (!is_file($this->_filePath)) {
                throw new \Exception('File ' . $this->_filePath . ' does not exist');
            }
            
            if (!is_readable($this->_filePath)) {
                throw new \Exception('Permission denied to read file ' . $this->_filePath);
            }
        }
