...
class CmsPageCreator
{
    ...
    public function createDefaultLexiconCmsPages(array $services, Context $context): void
    {
    ...
    private function setCmsSlot(string $type, string $slotName, ?string $languageEnglishId, ?string $languageGermanId, ?array $configGer = null, ?array $configEng = null): array
    {
        $slot = [];
        $slot['type'] = $type;
        $slot['slot'] = $slotName;
    ...
    private function setCmsBlock(string $type, int $position, string $sectionPosition, array $slots): array
    {
        $block = [];
    ...
    private function setCmsSection(string $type, int $position, array $blocks): array
    {
        $section = [];
        $section['type'] = $type;
    ...
    private function setCmsPage(string $type, array $sections, ?string $languageEnglishId, ?string $languageGermanId, ?string $nameGerman, ?string $nameEnglish): array
    {
        $page = [];
        $page['type'] = $type;
    ...
    public function deleteLexiconCmsPages(array $services, Context $context): void
    {
    ...
    public function deleteLexiconCmsBlocks(array $services, Context $context) : void
    {
    ...
    public function deleteLexiconCmsSlots(array $services, Context $context): void
    {
    ...
    public function updateLexiconCmsPages(array $services, $context): void
    {
        $oldVersion = $context->getCurrentPluginVersion();

        if (version_compare($oldVersion, '4.1.0', '<')) {
            $sql = "
                UPDATE `cms_slot_translation` SET `config` = JSON_SET(`config`, '$.buttonVariant', JSON_OBJECT('value', 'btn-outline-secondary', 'source','static'))
                WHERE cms_slot_id = (
                    SELECT id FROM `cms_slot` WHERE cms_block_id IN (
                        SELECT id FROM `cms_block` WHERE cms_section_id IN (
                            SELECT id FROM `cms_section` WHERE cms_page_id IN (
                                SELECT id FROM `cms_page` WHERE `type` = 'cbax_lexicon' AND `locked` = 1
                            )
                        )
                    ) 
                    AND type = 'cbax-lexicon-popular-entries'
                ) AND JSON_EXTRACT(config, '$.buttonVariant') IS NULL
            ;";
            ...
            $sql = "
                UPDATE `cms_slot_translation` SET `config` = JSON_SET(`config`, '$.buttonSize', JSON_OBJECT('value', 'btn-sm', 'source','static'))
                WHERE cms_slot_id = (
                    SELECT id FROM `cms_slot` WHERE cms_block_id IN (
                        SELECT id FROM `cms_block` WHERE cms_section_id IN (
                            SELECT id FROM `cms_section` WHERE cms_page_id IN (
                    ...
                    AND type = 'cbax-lexicon-popular-entries'
                ) AND JSON_EXTRACT(config, '$.buttonSize') IS NULL
            ;";
