...
class Updater
{
    public function updateData(ContainerInterface $container, UpdateContext $context): void
    {
        ...
        try {
            ...
            if (!$this->columnExist('cbax_lexicon_entry', 'media2_id', $connection)) {
                $connection->executeStatement("ALTER TABLE `cbax_lexicon_entry` ADD `media2_id` BINARY(16) DEFAULT NULL AFTER `attribute2`;");
            }

            if (version_compare($oldVersion, '4.0.7', '<')) {
                ...
                $this->addColumn($tableName, $columnName, $after, $setting, $connection);
            }
    ...
    public function afterUpdateSteps(array $services, UpdateContext $context)
    {
        ...
        try {
            if (version_compare($oldVersion, '3.1.3', '<')) {
                ...
                $systemConfigService->set(Database::PLUGIN_NAME . '.config.' . 'updateWithNotification',
                    $notification
                );
            }
        } catch (\Throwable $e) {
    ...
    public function addColumn($tableName, $columnName, $after, $setting, $connection) : void {
        if (!$this->columnExist($tableName, $columnName, $connection)){
            $connection->executeStatement("ALTER TABLE $tableName ADD $columnName ". $setting ." AFTER $after;");
        }
    }
}
