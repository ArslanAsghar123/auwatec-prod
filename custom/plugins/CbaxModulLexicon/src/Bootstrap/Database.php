...
class Database
{
    const PLUGIN_NAME = 'CbaxModulLexicon';
    const CONFIG_PATH = self::PLUGIN_NAME . '.config';

    public function updateDatabaseTables(array $services): void
    {
    ...
    public function deleteColumn(string $tableName, string $columnName, Connection $connection): void
    {
        if ($this->columnExist($tableName, $columnName, $connection)) {
            // delete Column
            $connection->executeStatement("ALTER TABLE `" . $tableName . "`  DROP COLUMN `" . $columnName . "`;");
