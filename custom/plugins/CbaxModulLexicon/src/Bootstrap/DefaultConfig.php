...
class DefaultConfig
{
     ...
     * Standardwerte
     */
    private array $defaults = ['activeProperties' => ['name', 'value']];

    private array $defaultCmsPageFields = [
    ...
    public function activate(array $services, Context $context): void
    {
        ...
        $shopwareConfig = $systemConfigService->get(Database::CONFIG_PATH) ?? [];
        ...
        foreach ($configs as $key => $config) {
            $systemConfigService->set(Database::CONFIG_PATH . '.' . $key, $config);
        }
        ...
        foreach ($this->defaultCmsPageFields as $key => $value) {
            if (empty($shopwareConfig[$key])) {
                ...
                if (!empty($pageId)) {
                    $systemConfigService->set(Database::CONFIG_PATH . '.' . $key, $pageId);
                }
            }
        }
    }
    ...
    public function checkSnippets(array $services): void
    {
        ...
        $oldBasisPath = $configService->get(Database::CONFIG_PATH . '.basisPath');
        $oldBasisPathContent = $configService->get(Database::CONFIG_PATH . '.basisPathContents');
        ...
        if (!empty($oldBasisPath)) {
            $configService->set(Database::CONFIG_PATH . '.basisPath', null);
        }

        if (!empty($oldBasisPathContent)) {
            $configService->set(Database::CONFIG_PATH . '.basisPathContents', null);
        }
    }
