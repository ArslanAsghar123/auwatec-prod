...
class FrontendController extends StorefrontController
{
    ...
    #[Route(path: '/cbax/lexicon/index', name: 'frontend.cbax.lexicon.index', options: ['seo' => true], defaults: ['auth_required' => false, '_httpCache' => true], methods: ['GET'])]
    public function index(Request $request, Context $context): Response
    {
        $salesChannelContext = $request->attributes->get(PlatformRequest::ATTRIBUTE_SALES_CHANNEL_CONTEXT_OBJECT);
        $salesChannelId = $salesChannelContext->getSalesChannelId();
        $this->config = $this->config ?? (array)$this->getSystemConfigService()->get(Database::CONFIG_PATH, $salesChannelId);
		...
		$breadcrumb[] = array(
            'link' => 'frontend.cbax.lexicon.index',
        ...
        if ($page->getMetaInformation()) {
            $robotsTag = $this->config['robotsTagIndex'] ?? 'index,follow';
            ...
            if (!empty($this->config['metaTitleExtended'])) {
                $metaTitle .= ' | ' . $this->getSystemConfigService()->getString('core.basicInformation.shopName', $salesChannelId);
            }
    ...
    #[Route(path: '/cbax/lexicon/listing/{char?}', name: 'frontend.cbax.lexicon.listing', options: ['seo' => true], defaults: ['auth_required' => false, '_httpCache' => true], methods: ['GET'])]
    public function listing(Request $request, Context $context): Response
    {
		...
		$breadcrumb[] = array(
            'link' => 'frontend.cbax.lexicon.listing',
            'name' => $char,
			'active' => true,
			'translated' => array(
				'name' => $char
        ...
        if ($page->getMetaInformation()) {
            ...
            $robotsTag = $this->config['robotsTagListing'] ?? 'index,follow';
            $page->getMetaInformation()->setRobots($robotsTag);
            ...
            if (!empty($this->config['metaTitleExtended'])) {
                $metaTitle .= ' | ' .
                    $this->getSnippet("cbaxLexicon.page.index.metaTitle", "Lexicon") .
                    ' | ' .
                    $this->getSystemConfigService()->getString('core.basicInformation.shopName', $salesChannelId);
            }
    ...
    #[Route(path: '/cbax/lexicon/modalInfo/{id?}', name: 'frontend.cbax.lexicon.modalInfo', options: ['seo' => false], defaults: ['XmlHttpRequest' => true, 'auth_required' => false, '_httpCache' => true], methods: ['GET'])]
    ...
    #[Route(path: '/cbax/lexicon/detail/{id?}', name: 'frontend.cbax.lexicon.detail', options: ['seo' => true], defaults: ['auth_required' => false, '_httpCache' => true], methods: ['GET'])]
    public function detail(Request $request, Context $context): Response
    {
        ...
        if (!empty($this->config['activeLexicon'])) {
            ...
            $translatedEntry['descriptionLong'] = $this->lexiconReplacer->getReplaceText($translatedEntry['descriptionLong'], $salesChannelId, $shopUrl, $context, $this->config, $lexiconEntryId);
            $translatedEntry['description'] = $this->lexiconReplacer->getReplaceText($translatedEntry['description'], $salesChannelId, $shopUrl, $context, $this->config, $lexiconEntryId);
		...
		$breadcrumb[] = array(
            'link' => 'frontend.cbax.lexicon.detail',
            'id' => $lexiconEntryId,
        ...
        if ($page->getMetaInformation()) {
            ...
            $robotsTag = $this->config['robotsTagDetail'] ?? 'index,follow';
            $page->getMetaInformation()->setRobots($robotsTag);
            ...
            $metaTitle = ($entry->getTranslated()['metaTitle'] ?? $entry->getTranslated()['title']);
            if (!empty($this->config['metaTitleExtended'])) {
                $metaTitle .= ' | ' .
                    $this->getSnippet("cbaxLexicon.page.listing.metaTitle" . $charForSnippet, $char) .
                    ' | ' .
                    $this->getSnippet("cbaxLexicon.page.index.metaTitle", "Lexicon") .
                    ' | ' .
                    $this->getSystemConfigService()->getString('core.basicInformation.shopName', $salesChannelId);
    ...
    #[Route(path: '/cbax/lexicon/content', name: 'frontend.cbax.lexicon.content', options: ['seo' => true], defaults: ['auth_required' => false, '_httpCache' => true], methods: ['GET'])]
    public function content(Request $request, Context $context): Response
    {
		...
		$linkContent = $this->lexiconSeo->getSeoUrl('frontend.cbax.lexicon.content' , '/cbax/lexicon/content', $context, $request);
		$breadcrumbLinkName = $this->container->get('translator')->trans('cbaxLexicon.page.breadcrumb.name');
		...
		$breadcrumb[] = array(
            'link' => 'frontend.cbax.lexicon.content',
        ...
        //Meta
        if ($page->getMetaInformation()) {
            $robotsTag = $this->config['robotsTagContent'] ?? 'index,follow';
            $page->getMetaInformation()->setRobots($robotsTag);
            ...
            $page->getMetaInformation()->setCanonical($linkContent);
        }
