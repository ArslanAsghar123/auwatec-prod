...
class LexiconCMSHelper
{
    ...
    public function filterEmptyCmsBlocks(LexiconPage $page): LexiconPage
    {
        ...
        if ($cmsPage->getSections()->getBlocks()?->count() == 0) {
            return $page;
        }
        ...
        foreach ($cmsPage->getSections() as $section) {
            ...
            if (empty($newBlocksCollection)) {
                continue;
            }

            foreach ($section->getBlocks() as $block) {
                if (in_array($block->getType(), $blocksNotToFilter)) {
                    continue;
                }
                if (
                    in_array($block->getType(),['youtube-video', 'vimeo-video']) &&
                    !empty($block->getSlots()?->first()?->getTranslated()['config']['videoID']['value'])
                ) {
                    continue;
                }
