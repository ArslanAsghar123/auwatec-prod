...
class LexiconHelper
{
    ...
    public function getLexiconEntryRepository() : EntityRepository
    {
        return $this->lexiconEntryRepository;
    }
    ...
    public function getSalesChannelRepository() : EntityRepository
    {
        return $this->salesChannelRepository;
    }

    public function getStorefrontSalesChannel($context){
    ...
    public function searchSuggestLexicon(string $term, int $searchLimit, Context $context): EntitySearchResult
    {
        $criteria = new Criteria();
        $criteria->setTerm($term);
        $criteria->setLimit($searchLimit);
        $criteria->setTotalCountMode(1);
        $criteria->addSorting(new FieldSorting('title', FieldSorting::ASCENDING));

        return $this->lexiconEntryRepository->search($criteria, $context);
    }
    ...
    public function saveEntry(array $entry, string $languageId, Context $context): array
    {
        ...
        if (!empty($entry['id'])) {
            ...
            $this->connection->executeStatement(
                "DELETE FROM `cbax_lexicon_sales_channel` WHERE `cbax_lexicon_entry_id` = ?",
                [UUid::fromHexToBytes($entry['id'])]
            );
        } else {
    ...
    public function doShopwareLog($content, $msg, $context, $type, $action)
    {
        $repo = $this->logEntryRepository;

        $this->logger->critical($msg, $content);

        $level = 400;
        if ($type == 'Info' || $type == 'Erfolgreich' || $type == 'Success') {
            $level = 200;
        }

        // neuen Log-Eintrag anlegen
        $repo->create(
            [
                [
                    'message' => $msg,
