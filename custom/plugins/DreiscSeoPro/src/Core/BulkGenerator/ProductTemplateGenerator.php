...
use DreiscSeoPro\Core\BulkGenerator\TemplateVariablesFetcher\ProductTemplateVariablesFetcher;
...
class ProductTemplateGenerator implements TemplateGeneratorInterface
{
    public function __construct(private readonly TemplateGeneratorHelper $templateGeneratorHelper, private readonly ProductTemplateVariablesFetcher $productTemplateVariablesFetcher, private readonly SeoUrlSlugify $seoUrlSlugify)
    {
    }
    ...
    public function generateTemplate(TemplateGeneratorStruct $templateGeneratorStruct, string $template, ?Entity $translatedEntity = null, Context $context = null): string
    {
        if (null === $translatedEntity || null === $context) {
            ...
            /** Make sure that the variant inheritance is activated */
            $languageChainContext->setConsiderInheritance(true);

            /** Fetch the template variables */
            $variables = $this->productTemplateVariablesFetcher->fetchVariables(
                $templateGeneratorStruct,
                $languageChainContext,
                $template
            );
        } else {
            $variables = $this->productTemplateVariablesFetcher->fetchVariablesByTranslatedProductEntity(
                $translatedEntity,
                $templateGeneratorStruct,
                $context,
                $template
