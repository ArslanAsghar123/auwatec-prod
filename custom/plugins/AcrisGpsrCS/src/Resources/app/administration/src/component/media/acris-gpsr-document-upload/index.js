import template from './acris-gpsr-document-upload.html.twig';
import './acris-gpsr-document-upload.scss';
 ...
 * </acris-document-upload>
 */
Component.register('acris-gpsr-document-upload', {
    template,
    ...
    methods: {
        ...
        addDownload(download) {
            ...
            const newDocument = this.productDownloadRepository.create(Shopware.Context.api);
            newDocument.mediaId = download.id;
            newDocument.download = {
                url: download.url,
                id: download.id,
                gpsrType: this.gpsrType
            };
            newDocument.gpsrType = this.gpsrType;
            this.product.extensions.acrisGpsrDownloads.add(newDocument);
        ...
        getDownloadDefaultFolderId() {
            return this.mediaDefaultFolderRepository.search(this.downloadDefaultFolderCriteria, Shopware.Context.api)
        ...
        async onUrlUpload({ url, fileExtension }) {
            ...
            await this.mediaRepository.save(targetEntity, Shopware.Context.api);
        ...
        async handleUpload(newMediaFiles) {
            ...
            await this.mediaRepository.saveAll(syncEntities, Shopware.Context.api);
        ...
        async getDefaultFolderId() {
            ...
            const items = await this.defaultFolderRepository.search(criteria, Shopware.Context.api);
            if (items.length !== 1) {
                return null;
            }
