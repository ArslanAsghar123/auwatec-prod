...
export default class GbmedForm extends Plugin {
    ...
    checkModalForms() {
        ...
        window.gbmedFormsOptions.modalForms.forEach(function(selector) {
            let formEl = document.querySelectorAll(selector);
            me.handleFormElement(formEl);
        });
    }

    handleFormElement(formEl) {
        const me = this;
        let form;
     ...
     */
    appendToken(form) {
        if (!this.isFormElement(form) || this.hasReCaptchaResponse(form)) {
            return;
        }
    ...
    formsExists() {
        ...
        if (!exist) {
            for (let selector of window.gbmedFormsOptions.modalForms) {
    ...
    existForm(el) {
        ...
        if (el && el.length) {
            ...
            return form && me.isFormElement(form);
        }
    ...
    hasReCaptchaResponse(form) {
        const reCaptchaBageHidden = form.elements['g-recaptcha-response'];

        return reCaptchaBageHidden !== undefined;
    }
}
